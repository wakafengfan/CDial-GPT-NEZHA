...
def _generate_relative_positions_embeddings(length, depth, max_relative_position):
...
class GenRelativePostionEmbed(nn.Module):
    def __init__(self, depth, max_relative_position):
        super(GenRelativePostionEmbed, self).__init__()
        vocab_size = max_relative_position * 2 + 1
        embeddings_table = np.zeros([vocab_size, depth])
        for pos in range(vocab_size):
            for i in range(depth // 2):
...
def _generate_relative_positions_embeddings_v2(depth, max_relative_position):
    vocab_size = max_relative_position * 2 + 1
    # range_vec = torch.arange(length)
    # range_mat = range_vec.repeat(length).view(length, length)
    # distance_mat = range_mat - torch.t(range_mat)
    ...
    embeddings_table_tensor = torch.tensor(embeddings_table).float()
    return embeddings_table_tensor
...
class NeZhaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        #
        # begin 构建新的相对位置向量
        self.relative_positions_embeddings = GenRelativePostionEmbed(
    ...
    def process_relative_position(self, seq_length, device):
        ...
        # one_hot_relative_positions_matrix = torch.nn.functional.one_hot(flat_relative_positions_matrix,
        #                                                                 num_classes=vocab_size).float()
        # embeddings = torch.matmul(one_hot_relative_positions_matrix.to(device), self.relative_positions_embeddings)
        embeddings = self.relative_positions_embeddings(flat_relative_positions_matrix)
        my_shape = list(final_mat.size())
        my_shape.append(depth)
        embeddings = embeddings.view(my_shape)
        relations_ = embeddings.clone().detach().to(device)
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        if head_mask is not None:
            attention_scores = sequence_masking(attention_scores, head_mask, '-inf', -1)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            hidden_states = layer_module(all_encoder_layers[i], attention_mask, head_mask)
